...
for i in d.properties.get('jobs'):
  for j in i:
    if  j == 'subrecords':  # It's a list of dicts.
      for n in i[j]:  # For each dict in the list.
        for p in n:  # For each key in the dict.
          ...
          s += t + fd  # The value
    if j == 'records':
      print(type(i[j]))  # It's a list of dicts, incl. subrecords.
      print(len(i[j]))
      print(i[j])  # There are subrecords in records. 
